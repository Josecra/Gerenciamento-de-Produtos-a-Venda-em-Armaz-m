...
const outListagem = document.getElementById("outListagem");
...
btListar.addEventListener("click", listarProdutosOrdenados);
btFiltro.addEventListener("click", filtrarProdutos);

function listarProdutosOrdenados() {
  let linhas = "";
  for (let i = 0; i < vetProdutos.length; i++) {
    linhas += `<tr><td>${vetProdutos[i].descricao}</td><td>${vetProdutos[i].qtEstoque}</td><td>${vetProdutos[i].preco.toFixed(2)}</td></tr>`;
  }
  outListagem.innerHTML = `<thead><tr><th>Descrição</th><th>Qt Estoque</th><th>Preço</th></thead><tbody>${linhas}</tbody>`;
}

function filtrarProdutos() {
  if (inFiltro.value == "") {
    alert("Informe o valor do filtro");
    inFiltro.focus();
  } else {
    var filtroSoma = 0;
    let linhas = "";
    var limite = Number(inFiltro.value);
    for (let i = 0; i < vetProdutos.length; i++) {
      if (vetProdutos[i].preco <= limite) {
        linhas += `<tr><td>${vetProdutos[i].descricao}</td><td>${vetProdutos[i].qtEstoque}</td><td>${vetProdutos[i].preco.toFixed(2)}</td></tr>`;
        filtroSoma += vetProdutos[i].qtEstoque * vetProdutos[i].preco;
      }
    }
    outListagem.innerHTML = `<thead><tr><th>Descrição</th><th>Qt Estoque</th><th>Preço</th></thead><tbody>${linhas}</tbody>`;
    outTotal.innerHTML = "O valor total dos produtos filtrados é de: " + filtroSoma.toFixed(2);
  }
}